            ...
            <div class="p-6 border-b border-gray-200">
                <div class="md:flex items-center border-b bg-gray-50 px-5 py-1 md:justify-between font-semibold">
                    ...
                    <div class="text-xs border-b bg-gray-50 text-gray-500">
                        <select id="cantidad" v-model="itemsPerPage" class="block md:w-18 md:h-10 md:text-sm md:mt-0 w-18 h-8 text-xs mt-2 border border-gray-300 rounded-md" required>
                ...
                <TablePerson :users=slicedEstudiantes />
...
//Paginacion y filtro
const slicedEstudiantes = computed(() => {
    const start = currentPage.value * itemsPerPage.value;
    const end = start + itemsPerPage.value;
    const filteredUsers = usePage().props.users.filter(estudiante =>
        estudiante.name.toLowerCase().includes(searchTerm.value.toLowerCase()) ||
        estudiante.email.toLowerCase().includes(searchTerm.value.toLowerCase())
    );
