    ...
    <AuthenticatedLayout>
        ...
        <div class="md:inline-block min-w-full overflow-x-scroll rounded-lg shadow">
            <table class="w-full whitespace-no-wrap overflow-x-auto">
                ...
                <tbody>
                    <tr v-for="user in slicedUsers" :key="user.id" class="text-gray-700">
                        ...
                        <td class="border-b border-gray-200 bg-white sm:px-5 sm:py-5 p-2 sm:text-sm text-xs">
                            <Link :href="route('profile.edit', { id: user.id, originPage: 'UsuariosEdit' })"
                                class="bg-cyan-600 md:mr-3 text-white xs:text-xs sm:px-4 sm:py-2 p-2 bg-blue-600 rounded-lg">
                            Editar</Link>

                            <!-- <Link @click="destroy(blog.id)"
                                class="bg-cyan-600 md:mr-3 text-white xs:text-xs sm:px-4 sm:py-2 p-2 bg-blue-600 rounded-lg">
                            Eliminar</Link> -->
                        </td>
                    </tr>
...
const slicedUsers = computed(() => {
    ...
    const filteredUsers = props.users.filter(monitor =>
